...
void reset(){
  StepperZ.step((Z_Rotation/StepsPerRev)*-4);
  StepperY.step((Y_maxRotation - Y_Rotation/GradProStep) *-4);
  Y_Rotation = Y_maxRotation;
  Z_Rotation = 0; 
}

void automate(){
  if(NDMI < minNDMI){
    StepperA.step((Z_Rotation/StepsPerRev)*4);
    if(Messradius <= 1.18){
      Servo1.write(90);
      delay(2000);
      Servo1.write(-90);
